...
func TestSortByLength(t *testing.T) {
	got := SortByLength([]string{"banana", "fig", "apple", "kiwi"})
...
func TestSortByAbsValue(t *testing.T) {
	got := SortByAbsValue([]int{-3, 1, -2, 4})
	want := []int{1, -2, -3, 4}
	if !reflect.DeepEqual(got, want) {
...
func TestMedianSorted(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("Median(%v)", tt.nums), func(t *testing.T) {
			if got := MedianSorted(tt.nums); got != tt.want {
...
func TestBinarySearch(t *testing.T) {
	s := []int{1, 3, 5, 7, 9, 11}
	if got := BinarySearch(s, 7); got != 3 {
		t.Errorf("BinarySearch(7) = %d, want 3", got)
	}
	if got := BinarySearch(s, 4); got != -1 {
