...
func main() {
	fmt.Printf("%s%s══════════════════════════════════════════%s\n", bold, blue, reset)
	fmt.Printf("%s%s  Built-in Functions                      %s\n", bold, blue, reset)
	fmt.Printf("%s%s══════════════════════════════════════════%s\n\n", bold, blue, reset)
...
func demonstrateMake() {
	fmt.Printf("%s▸ 1. make() — Slices, Maps, Channels%s\n", cyan+bold, reset)
	fmt.Printf("  %s✔ make() is ONLY for slices, maps, and channels%s\n", green, reset)
	fmt.Printf("  %s✔ It allocates AND initializes the internal structure%s\n", green, reset)
...
func demonstrateNew() {
	fmt.Printf("%s▸ 2. new() — Zeroed Memory Pointer%s\n", cyan+bold, reset)
	fmt.Printf("  %s✔ new(T) allocates zeroed memory, returns *T%s\n", green, reset)
	fmt.Printf("  %s⚠ Less common than composite literals — prefer &T{} in most cases%s\n", yellow, reset)
...
func demonstrateAppend() {
	fmt.Printf("%s▸ 3. append() — Add Elements to a Slice%s\n", cyan+bold, reset)
	fmt.Printf("  %s✔ Always reassign: s = append(s, elem) — may return new backing array%s\n", green, reset)
	...
	fmt.Printf("  %s⚠ Sub-slice append gotcha — shared backing array%s\n", yellow, reset)
	a := []int{1, 2, 3, 4, 5}
...
func demonstrateCopy() {
	fmt.Printf("%s▸ 4. copy() — Copy Between Slices%s\n", cyan+bold, reset)
	fmt.Printf("  %s✔ copy(dst, src) copies min(len(dst), len(src)) elements%s\n", green, reset)
	...
	byt := make([]byte, 5)
	copy(byt, "Hello")
	fmt.Println("  string→[]byte:", byt) // [72 101 108 108 111]
	...
	fmt.Printf("  %s⚠ Deep copy 2D slice: must copy each row individually%s\n", yellow, reset)
	matrix := [][]int{{1, 2}, {3, 4}}
...
func demonstrateLenCap() {
	fmt.Printf("%s▸ 5. len() and cap()%s\n", cyan+bold, reset)
	fmt.Printf("  %s✔ len(x) = current elements, cap(x) = total capacity%s\n", green, reset)
	fmt.Printf("  %s✔ Works on: strings, arrays, slices, maps, channels%s\n", green, reset)
	...
	for i := 0; i < 10; i++ {
		s2 = append(s2, i)
		fmt.Printf("  len=%s%d%s cap=%s%d%s\n", magenta, len(s2), reset, magenta, cap(s2), reset)
...
func demonstrateDelete() {
	fmt.Printf("%s▸ 6. delete() — Remove Map Keys%s\n", cyan+bold, reset)
	fmt.Printf("  %s✔ delete(map, key) — no-op if key doesn't exist%s\n", green, reset)
...
func demonstrateConversions() {
	fmt.Printf("%s▸ 7. Type Conversions%s\n", cyan+bold, reset)
	fmt.Printf("  %s✔ Go requires EXPLICIT conversions — no implicit casting%s\n", green, reset)

	// int ↔ float64
	var i int = 42
	var f float64 = float64(i)
	var i2 int = int(f) // truncates (does NOT round)
	fmt.Println("  int→float64→int:", i, f, i2)
	...
	fmt.Printf("  %s⚠ string(65) = %q — character, not number! Use strconv.Itoa()%s\n", yellow, string(rune(65)), reset)
...
func demonstratePanicRecover() {
	fmt.Printf("%s▸ 8. panic() and recover()%s\n", cyan+bold, reset)
	fmt.Printf("  %s⚠ Use sparingly! Prefer returning errors. Panic = unrecoverable states only%s\n", yellow, reset)
